...
function brightest(colors) {
  let brightestInsideColor = 0;
  let indexBrightestColor = 0;
  for (let i = 0; i < colors.length; i++) {
    const r = parseInt(colors[i].slice(1, 3), 16);
    const g = parseInt(colors[i].slice(3, 5), 16);
    const b = parseInt(colors[i].slice(5, 7), 16);
    if (Math.max(r, g, b) > brightestInsideColor) {
      brightestInsideColor = Math.max(r, g, b);
      indexBrightestColor = i;
    }
  }
  return colors[indexBrightestColor];
}
